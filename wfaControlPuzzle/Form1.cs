namespace wfaControlPuzzle
{
    public partial class Form1 : Form
    {
        ...
        private void Random2CellLocation()
        {
            StartLocationCells();
            for (int i = 0;i < 10; i++) {
        ...
        private void RandomCellLocation()
        {
            for (int r = 0; r < Rows; r++)
            {
                for (int c = 0; c < Cols; c++)
                {
                    px[r, c].Location = new Point(rnd.Next(this.ClientSize.Width - px[r, c].Width), rnd.Next(this.ClientSize.Height - px[r,c].Height)) ;
                }
            }
        }
            ...
            if (sender is Control v)
            {
                ...
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    for (int r = 0; r < Rows; r++)
                    {
                        for (int c = 0; c < Cols; c++)
                        {
                            if (p.X > c * cellWidth - STEP && p.X < c * cellWidth + 10)
                            {
                                p.X = c*cellWidth;
                            }
                            if (p.Y > r * cellHeight - STEP && p.Y < r * cellHeight + 10)
                            {
                                p.Y = r * cellHeight;
                            }
                            //px[r, c].Location = new Point(rnd.Next(this.ClientSize.Width - px[r, c].Width), rnd.Next(this.ClientSize.Height - px[r, c].Height));
